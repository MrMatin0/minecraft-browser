...
MC.Shared.push(function defineNoise(ns) {
  ...
  var F3 = 1 / 3, G3 = 1 / 6;
  // Gradient table flattened into a typed array: GRAD[gi * 3 + axis].
  // The old array-of-arrays version cost a pointer chase per component and this is
  // the single hottest inner loop in world generation.
  var GRAD = new Int8Array([
  ...
  Noise.prototype.noise2D = function (xin, yin) {
    var perm = this.perm, permMod12 = this.permMod12, g = GRAD;
    var n0 = 0, n1 = 0, n2 = 0;
    ...
    if (t0 > 0) { var k0 = permMod12[ii + perm[jj]] * 3; t0 *= t0; n0 = t0 * t0 * (g[k0] * x0 + g[k0 + 1] * y0); }
    var t1 = 0.5 - x1 * x1 - y1 * y1;
    if (t1 > 0) { var k1 = permMod12[ii + i1 + perm[jj + j1]] * 3; t1 *= t1; n1 = t1 * t1 * (g[k1] * x1 + g[k1 + 1] * y1); }
    var t2 = 0.5 - x2 * x2 - y2 * y2;
    if (t2 > 0) { var k2 = permMod12[ii + 1 + perm[jj + 1]] * 3; t2 *= t2; n2 = t2 * t2 * (g[k2] * x2 + g[k2 + 1] * y2); }
    return 70 * (n0 + n1 + n2);
  };
  Noise.prototype.noise3D = function (xin, yin, zin) {
    ...
    if (t0 > 0) { var m0 = permMod12[ii + perm[jj + perm[kk]]] * 3; t0 *= t0; n0 = t0 * t0 * (g[m0] * x0 + g[m0 + 1] * y0 + g[m0 + 2] * z0); }
    var t1 = 0.6 - x1 * x1 - y1 * y1 - z1 * z1;
    if (t1 > 0) { var m1 = permMod12[ii + i1 + perm[jj + j1 + perm[kk + k1]]] * 3; t1 *= t1; n1 = t1 * t1 * (g[m1] * x1 + g[m1 + 1] * y1 + g[m1 + 2] * z1); }
    var t2 = 0.6 - x2 * x2 - y2 * y2 - z2 * z2;
    if (t2 > 0) { var m2 = permMod12[ii + i2 + perm[jj + j2 + perm[kk + k2]]] * 3; t2 *= t2; n2 = t2 * t2 * (g[m2] * x2 + g[m2 + 1] * y2 + g[m2 + 2] * z2); }
    var t3 = 0.6 - x3 * x3 - y3 * y3 - z3 * z3;
    if (t3 > 0) { var m3 = permMod12[ii + 1 + perm[jj + 1 + perm[kk + 1]]] * 3; t3 *= t3; n3 = t3 * t3 * (g[m3] * x3 + g[m3 + 1] * y3 + g[m3 + 2] * z3); }
